...
describe("isDateInSeasonPeriod", () => {
  ...
  it("returns true at the exact start boundary", () => {
    expect(
      isDateInSeasonPeriod(
        new Date(regularSeason2026.startDate),
        regularSeason2026,
      ),
    ).toBe(true);
  });
  ...
  it("returns true at the exact end boundary", () => {
    expect(
      isDateInSeasonPeriod(
        new Date(regularSeason2026.endDate),
        regularSeason2026,
      ),
    ).toBe(true);
  });
...
describe("getWeekForDate", () => {
  ...
  it("returns the correct week number", () => {
    expect(
      getWeekForDate(new Date("2026-09-01T12:00:00.000Z"), regularSeason2026),
    ).toBe(1);
    expect(
      getWeekForDate(new Date("2026-09-10T12:00:00.000Z"), regularSeason2026),
    ).toBe(2);
    expect(
      getWeekForDate(new Date("2026-09-17T12:00:00.000Z"), regularSeason2026),
    ).toBe(3);
  });
  ...
  it("returns null for a date that falls in no defined week", () => {
    ...
    const oneWeek = {
      ...regularSeason2026,
      weeks: regularSeason2026.weeks!.slice(0, 1),
    };
    expect(getWeekForDate(new Date("2026-09-10T12:00:00.000Z"), oneWeek)).toBe(
      null,
    );
  });
  ...
  it("returns null when weeks is undefined", () => {
    ...
    expect(getWeekForDate(new Date("2026-09-01T12:00:00.000Z"), noWeeks)).toBe(
      null,
    );
  });
});
  ...
  it("returns empty arrays when no season type has weeks", async () => {
    const noWeeks = {
      ...
      seasons: [
        {
          ...season2026,
          types: season2026.types.map((t) => ({ ...t, weeks: undefined })),
        },
      ],
    };
