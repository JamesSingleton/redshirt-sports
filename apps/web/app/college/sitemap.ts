...
import type { QueryForCollegeSitemapResult } from "@redshirt-sports/sanity/types";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const { data } = (await sanityFetchMetadata({
    query: queryForCollegeSitemap,
    perspective: "published",
  })) as { data: QueryForCollegeSitemapResult | null };
  const posts = data ?? [];
  ...
  posts.forEach((post) => {
    ...
    const isD1 =
      post.division &&
      (divisionLower === "division-i" || divisionLower === "d1");
    ...
    if (divisionSegment) {
      ...
      const url2 = `${baseUrl}/college/${sport}/news/${divisionSegment}`;
      ...
      if (post.conferences && post.conferences.length > 0) {
        post.conferences
          ...
          .forEach((conference) => {
            ...
            const url3 = `${baseUrl}/college/${sport}/news/${divisionSegment}/${conference.slug}`;
