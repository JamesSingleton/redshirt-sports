...
describe("formatPublishDate", () => {
  it("shows compact minutes for posts under an hour old", () => {
    expect(
      formatPublishDate("2026-06-24T19:15:00.000Z", now),
    ).toBe("45m");
  });
  ...
  it("shows days ago for posts between 1 and 4 days old", () => {
    expect(formatPublishDate("2026-06-23T20:00:00.000Z", now)).toBe("1 day ago");
    expect(formatPublishDate("2026-06-22T20:00:00.000Z", now)).toBe("2 days ago");
    expect(formatPublishDate("2026-06-20T20:00:00.000Z", now)).toBe("4 days ago");
  });
