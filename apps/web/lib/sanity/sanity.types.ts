...
// Query: *[_type == "post" && defined(slug.current)]| order(publishedAt desc)[0...50].slug.current
export type QueryPostPathsResult = Array<string>
// Variable: queryAuthorPaths
// Query: *[_type == "author" && defined(slug.current) && archived == false]| order(_createdAt desc)[0...20].slug.current
export type QueryAuthorPathsResult = Array<string>
...
// Variable: schoolsBySportAndSubgroupingWorkingQuery
// Query: *[_type == "school" &&    top25VotingEligible != false &&    count(conferenceAffiliations[sport->slug.current == $sport]) > 0  ] {    _id,    name,    shortName,    abbreviation,    image,    "sportConferenceAffiliation": conferenceAffiliations[sport->slug.current == $sport][0] {      sport-> {        title,        slug      },      conference-> {        _id,        name,        shortName,        abbreviation,        "validSubgrouping": sportSubdivisionAffiliations[          sport->slug.current == $sport &&          subgrouping->slug.current == $subgrouping        ][0]      }    }  }[defined(sportConferenceAffiliation.conference.validSubgrouping)] | order(shortName asc) {    _id,    name,    shortName,    abbreviation,    image,    "conferenceInfo": {      "conference": sportConferenceAffiliation.conference {        name,        shortName,        abbreviation      }    }  }
export type SchoolsBySportAndSubgroupingWorkingQueryResult = Array<{
  _id: string
  name: string
  shortName: string | null
  abbreviation: string | null
...
// Query: *[_type == "school" &&    top25VotingEligible != false  ] {    _id,    name,    shortName,    abbreviation,    image,    conferenceAffiliations,    "relevantAffiliation": conferenceAffiliations[sport->slug.current == $sport][0]  }[defined(relevantAffiliation)] {    _id,    name,    shortName,    abbreviation,    image,    relevantAffiliation,    "conferenceDetails": *[_type == "conference" && _id == ^.relevantAffiliation.conference._ref][0] {      name,      shortName,      abbreviation,      sportSubdivisionAffiliations    }  }[    count(conferenceDetails.sportSubdivisionAffiliations[      sport->slug.current == $sport &&      subgrouping->slug.current == $subgrouping    ]) > 0  ] | order(shortName asc) {    _id,    name,    shortName,    abbreviation,    image,    "conferenceInfo": {      "conference": conferenceDetails {        name,        shortName,        abbreviation      }    }  }
