    ...
    <header className="sm:px-10 sm:pt-10">
      ...
      <div className="px-5 lg:px-0">
        <div className="mx-auto mb-8 max-w-prose border-b border-zinc-300/70 pb-8 pt-10 text-lg sm:pt-16">
          ...
          <h1 className="mt-3 font-cal text-4xl font-medium text-zinc-900 transition duration-300 ease-in-out sm:my-5 sm:text-4xl sm:leading-tight lg:text-5xl">
            {post.title}
          </h1>
          <p className="mt-4 text-base leading-loose text-zinc-600">{post.excerpt}</p>
          <div className="mt-6 flex items-center sm:mt-8">
            ...
            <div className="flex items-center text-sm lg:text-base">
              <span className="hidden text-zinc-500 sm:inline-block">By&nbsp;</span>
              <Link
              ...
              <Date className="ml-1 text-zinc-500" dateString={post.publishedAt} />
              <span className="hidden items-center sm:flex">
                <ClockIcon className="ml-3 h-5 w-5 text-zinc-400" />
                <span className="ml-1 text-zinc-500">{`${post.estimatedReadingTime} min read`}</span>
              </span>
            </div>
          </div>
        </div>
