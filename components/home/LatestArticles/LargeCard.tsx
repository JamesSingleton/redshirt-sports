...
const LargeCard: FC<LargeCardProps> = ({ post }) => {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-700 group relative flex flex-col overflow-hidden h-full rounded-md">
      ...
      <div className="p-4 sm:p-5 flex flex-col">
        <div className="space-y-3">
          ...
          <h2 className="block font-semibold text-slate-900 dark:text-slate-50 transition-colors text-lg sm:text-2xl">
            <Link href={`/${post.slug}`}>
              <a className="line-clamp-2">{post.title}</a>
            </Link>
          </h2>
          <span className="block text-sm line-clamp-2">{post.excerpt}</span>
        </div>
        <a
          className="relaative inline-flex items-center relative my-4"
          href={`/${post.slug}`}
        >
