...
export default function Tweet({ id, metadata, className }: TweetProps) {
  ...
  return (
    <div
      className={`${className} tweet my-4 w-full rounded-lg border border-zinc-300 bg-white px-8 pb-2 pt-6`}
    >
      <div className="flex items-center">
        ...
        >
          <span
            className="mt-1 flex items-center font-bold leading-5 text-zinc-900"
            title={author.name}
          >
            {author.name}
            {author.verified ? (
          ...
          <span className="text-base !text-zinc-500" title={`@${author.username}`}>
            @{author.username}
          </span>
        </a>
        <a
      ...
      <div
        className="mb-2 mt-4 whitespace-pre-wrap text-lg leading-normal text-zinc-700"
      ...
      {url_meta?.images ? (
        <a className="!no-underline" href={url_meta.unwound_url} rel="noreferrer" target="_blank">
          <div className="mb-5 overflow-hidden rounded-2xl border border-zinc-200 drop-shadow-sm">
            <BlurImage
                ...
                {poll.options.map((option) => (
                  <>
                    ...
                    >
                      ...
                      <div
                        className={classNames(
                          option.position == 1 ? 'bg-[#1da1f2] font-bold' : 'bg-zinc-300',
      ...
      <a
        className="mb-4 mt-3 block text-base !text-zinc-500 !no-underline hover:!underline"
        href={tweetUrl}
        target="_blank"
        rel="noopener noreferrer"
      >
      ...
      <div className="mt-2 flex space-x-1 border-t border-zinc-300 pt-1 text-base text-zinc-700 md:space-x-6">
        <a
          className="group flex items-center -space-x-0.5 !text-zinc-500 !no-underline transition"
          href={likeUrl}
          target="_blank"
          rel="noopener noreferrer"
        >
        ...
        <a
          className="group mr-4 flex items-center -space-x-0.5 !text-zinc-500 !no-underline transition"
          href={retweetUrl}
          target="_blank"
          rel="noopener noreferrer"
        >
