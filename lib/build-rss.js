...
import { postsForRssFeed } from './sanity.queries'
...
const myPortableTextComponents = {
  types: {
    image: ({ value }) => html`<img src="${urlForImage(value)}" />`,
  ...
  marks: {
    ...
    link: ({ value, children }) => {
      ...
      if (uriLooksSafe(href)) {
        ...
        const finalHref = href.startsWith('/') ? `${baseUrl}${href}` : href
        return html`<a href="${finalHref}" rel="${rel}">${children}</a>`
      }
...
export default async function buildRss() {
  const posts = await client.fetch(postsForRssFeed, {}, { perspective: 'published' })
