...
export default async function preview(req: NextApiRequest, res: NextApiResponse<string | void>) {
  ...
  const href = resolveHref(req.query.documentType as string, req.query.slug as string)

  if (!href) {
    return res
      .status(400)
      .send('Unable to resolve preview URL based on the current document type and slug')
  }
